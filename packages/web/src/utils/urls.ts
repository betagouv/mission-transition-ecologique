...
export const unfoldQueries = (query: any) => {
  ...
  // console.log('utils > urls > unfoldQueries > queryTracksRaw :', queryTracksRaw)
  
  const queryTracks = Object.keys(queryTracksRaw)
  // console.log('utils > urls > unfoldQueries > queryTracks :', queryTracks)
  ...
  const tracksArray: any[] = queryTracks.map(key => {
    ...
    // console.log('utils > urls > unfoldQueries > strParts :', strParts)
    ...
    // console.log('utils > urls > unfoldQueries > pairs :', pairs)
    obj.selected = pairs.filter((i: any) => !!i)
    return obj
  })
  console.log('utils > urls > unfoldQueries > tracksArray :', tracksArray)
  return tracksArray
}