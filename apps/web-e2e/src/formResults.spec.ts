...
tests.forEach((singleTest) => {
  test(`Test id ${singleTest.id} - Verify form ${singleTest.url}`, async ({ page }) => {
    ...
    for (const [fieldKey, value] of Object.entries(singleTest.values)) {
      ...
      if (fieldKey === 'siret') {
        if (singleTest.manual) {
          ...
          const actualSiretValue = await page.inputValue(selector, {timeout: 1000})
          expect(actualSiretValue).toBe(value.value)
        }
      } else if (fieldKey === 'needs') {
        const actualNeedsValue = await page.inputValue(selector, {timeout: 1000})
        ...
        await page.locator(selector).fill(value.value as string, {timeout: 1000})
      } else if (value.type === 'select' && singleTest.type === 'customProject') {
        await page.locator(selector).selectOption({ label: value.value as string }, {timeout: 1000})
    ...
    await expect(page.locator('button[type="submit"]')).toHaveCount(1, { timeout: 3000 })
    if (singleTest.valid) {
