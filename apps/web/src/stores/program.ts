...
import { type programFiltersType, ProgramAidType, ThemeId, Region, type ProgramData, QuestionnaireData } from '@/types'
  ...
  function getProgramsByFilters(programs: ProgramData[]) {
    return programs.filter((program: ProgramData) => {
      return (
        ...
        ProgramFilter.byOperator(program, programFilters.value.operatorAidSelected) &&
