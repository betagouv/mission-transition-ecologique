...
const projetsFiltres = computed({
  get: () => {
    ...
    // 1. On vérifie si c'est un tableau (cas des relations multiples)
    if (Array.isArray(rawValue) && rawValue.length > 0) {
      // On récupère la valeur du premier élément
      const stringValue = rawValue[0].value; 
      return stringValue === selectedNaf.value;
    }

    // 2. Cas de secours (si jamais c'est un objet simple ou une string)
    const stringValue = typeof rawValue === 'object' ? rawValue?.value : rawValue;
    return stringValue === selectedNaf.value;
  })
    )
  },
