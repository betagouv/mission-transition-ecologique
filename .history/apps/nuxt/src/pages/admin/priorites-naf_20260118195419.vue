...
const projetsFiltres = computed({
  get: () => {
    ...
    return sortProjectsByPriority(
      base.filter(p => {
        const field = p["Thématique principale"];
        // On récupère la string, peu importe si c'est un objet ou direct
        const stringValue = typeof field === 'object' ? field?.value : field;
        
        if (!stringValue) return false;

        // Comparaison insensible à la casse et aux espaces
        return stringValue.toLowerCase().trim() === selectedNaf.value.toLowerCase().trim();
      })
    )
  },
  set: (newOrder) => {
