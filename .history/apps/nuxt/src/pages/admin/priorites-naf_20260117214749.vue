...
<template>
  <div class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    <main class="mt-8">
      ...
      <div v-else-if="error" class="bg-red-50 border-l-4 border-red-500 p-4 rounded shadow-sm text-red-700">
        <p class="font-bold">Erreur de connexion</p>
        <p>{{ error.statusMessage || "Impossible de récupérer les projets." }}</p>
      </div>

      <div v-else>
        <ListeProjets :projets="projetsFiltres" />
        
        <div class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200 flex justify-between items-center text-sm text-gray-600">
          <div>
            Total affiché : <strong>{{ projetsFiltres.length }}</strong> projet(s)
            <span v-if="selectedNaf" class="ml-1 text-blue-600 font-medium">
              (Secteur {{ selectedNaf.match(/[A-Z]$/)?.[0] }})
            </span>
          </div>
          <div v-if="projects" class="text-xs text-gray-400 italic">
            Source : {{ projects.length }} projets chargés en mémoire
          </div>
        </div>
      </div>
    </main>
