...
const projetsFiltres = computed({
  ...
  base.filter(p => {
    const thematique = p["Thématique principale"];
    const label = (typeof thematique === 'object' ? thematique?.value : thematique) || "";
    
    // On compare tout en minuscules et sans espaces inutiles
    return label.toString().toLowerCase().trim() === selectedNaf.value.toLowerCase().trim();
  })
)
  },
  set: (newOrder) => {
