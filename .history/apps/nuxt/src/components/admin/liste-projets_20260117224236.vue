...
const emit = defineEmits(['update:order', 'change'])

// On crée une liaison calculée pour le draggable
const list = computed({
  get: () => props.projets,
  set: (value) => {
...
<template>
  <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
    ...
    <div v-if="!projets || projets.length === 0" class="p-10 text-center text-gray-500 italic">
      Aucun projet trouvé.
    </div>
  </div>
</template>