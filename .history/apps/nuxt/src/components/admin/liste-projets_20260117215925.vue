...
<template>
  ...
  {{ projet.Titre || projet.title || 'Projet sans nom' }}
</td>
<td class="px-4 py-2">
  <span
    class="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium"
    :class="projet.status ? (projet.status === 'En prod' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800') : 'bg-red-100 text-red-800'"
  >
    {{ projet.status || 'Statut manquant' }}
  </span>
</td>
